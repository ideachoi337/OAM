...
class FullVideoDataSet(data.Dataset):
    ...
    def _makeInputSeq(self):
        ...
        for index in range(0,len(self.video_list)):
            ...
            self.inputs_all.append([video_name, 0, duration, data_idx])
            data_idx+=1
    ...
    def _makePropLabelUnit(self, i):
        video_name=self.inputs_all[i][0] # video_name
        vid_duration = self.inputs_all[i][2] # end time of video

        vid_cls_anc = []
        vid_reg_anc = []

        for t in range(1, vid_duration+1):
            # now, st = 0 and ed = t
            st = 0; ed = t
            cls_anc = []
            reg_anc = []
            for j in range(len(self.anchors)):
                ...
                cls_anc.append(v1)
                reg_anc.append(v2)
            vid_cls_anc.append(np.stack(cls_anc, axis=0)) # (n_anchors, n_class)
            vid_reg_anc.append(np.stack(reg_anc, axis=0)) # (n_anchors, 2)

        vid_cls_anc = np.stack(vid_cls_anc, axis=0) # (vid_len, n_anchors, n_class)
        vid_reg_anc = np.stack(vid_reg_anc, axis=0) # (vid_len, n_anchors, 2)

        return vid_cls_anc, vid_reg_anc
    
    def _loadPropLabel(self, filename):
        if os.path.exists(filename+'_cls.pkl'):
            self.cls_label=pickle.load(open(filename+'_cls.pkl', 'rb'))
            self.reg_label=pickle.load(open(filename+'_reg.pkl', 'rb'))
            return
        ...
        self.cls_label=[] # length: num_vid, (vid_len, n_anchors, n_class)
        self.reg_label=[] # length: num_vid (vid_len, n_anchors, 2)
        for i in range(0,len(labels)):
            self.cls_label.append(labels[i][0])
            self.reg_label.append(labels[i][1])

        with open(filename+'_cls.pkl', 'wb') as f:
            pickle.dump(self.cls_label, f)
        with open(filename+'_reg.pkl', 'wb') as f:
            pickle.dump(self.reg_label, f)
    ...
    def __getitem__(self, index):
        ...
        return feature,cls_label,reg_label # (vid_len, feature_dim), (vid_len, n_anchors, n_class), (vid_len, n_anchors, 2)
