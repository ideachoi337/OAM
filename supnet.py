...
from dataset import FullVideoDataSet, SuppressDataSet
from models import MambaNet, SuppressNet
...
def train(opt): 
    ...
    for n_epoch in range(opt['epoch']):   
        ...
        torch.save(state, opt["checkpoint_path"]+f"/checkpoint_suppress_{'stg2' if opt['stage2'] else 'stg1'}.pth.tar" )
        if eval_cost < model.best_loss:
            model.best_loss = eval_cost
            torch.save(state, opt["checkpoint_path"]+f"/ckp_best_suppress_{'stg2' if opt['stage2'] else 'stg1'}.pth.tar" )
