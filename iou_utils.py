...
def non_max_suppression(proposals, overlapThresh=0.3):
    ...
    while idx < total_proposal: 
        ...
        st = np.float32(proposal['segment'][0])
        ed = np.float32(proposal['segment'][1])
        label = proposal['label']
        
        delete_item = []
        for j in range(idx+1, total_proposal):
            target_proposal = sorted_proposal[j]
            target_st = np.float32(target_proposal['segment'][0])
            target_ed = np.float32(target_proposal['segment'][1])
